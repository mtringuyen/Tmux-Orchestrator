...
echo "=== Next Check Note ($(date)) ===" > /home/tri/Coding/Tmux\ orchestrator/next_check_note.txt
echo "Scheduled for: $MINUTES minutes" >> /home/tri/Coding/Tmux\ orchestrator/next_check_note.txt
echo "" >> /home/tri/Coding/Tmux\ orchestrator/next_check_note.txt
echo "$NOTE" >> /home/tri/Coding/Tmux\ orchestrator/next_check_note.txt
...
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat /home/tri/Coding/Tmux\ orchestrator/next_check_note.txt && python3 claude_control.py status detailed' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
